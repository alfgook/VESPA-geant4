...
void ScintilatorHit::AddLight(G4double dL, G4double steptime)
{
	//set time-of-flight for the step with the highest light yield 
	//if(dL>fLight) fTime = steptime; 
	if((steptime<fTime || fTime==0)) fTime = steptime; 
	fLight += dL;
}
